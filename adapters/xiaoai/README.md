...
| `EV_KEEP_MIC` | `0` | 答完自动开麦（连续对话）。**默认关闭**，见下方说明 |
...
- **打断要连打，而且要抢在它开口前**。实测时序：`is_final` 之后小爱**立刻**
  `StartAnswer` 并连说两句（IoT 指令走云端直连，比我们快），
...
- **⚠️ 千万别用 `pnshelper event 7` 打断说话。** 它是「取消唤醒」序列的**前半截**
  ...
  **实测导致唤醒词彻底失灵，只能手动点麦克风**。
  ...
  它只动音频、不碰唤醒状态机。
- **连续对话（EV_KEEP_MIC=1，默认关）**：答完用
  `pnshelper event_notify '{"src":1,"event":0}'` 静默开麦（`src=1` 不出提示音）。
  机制本身验证有效——发完这条不喊唤醒词直接说话能识别。

  **但默认关掉，因为有真实副作用**：麦持续开着会把环境噪音识别成碎片文本
  （实测收到过「没」「了」这种单字），然后触发一轮回答、又开麦，
  陷入「开麦 → 收噪音 → 回答 → 又开麦」的循环。要用请配合过短过滤。
- **无分隔符的连续动词要按动词边界切**。「开窗帘关灯」这种口语很常见，
  没有逗号也没有连接词，靠标点/连接词切不开，整句问模型会判反
  （实测「嗯开窗帘关灯」→ `curtain_close`，而分开问 `开窗帘`/`关灯` 都对）。
- **过短识别必须过滤**。判断字节长度，**不能用 `case "$text" in ?|??)`**——
  shell 的 `?` 匹配单**字节**，一个中文字是 3 字节，永远匹配不上（我第一版就错在这）。
  正确写法：`BYTES=$(printf %s "$text" | wc -c); [ "$BYTES" -le 6 ] && continue`

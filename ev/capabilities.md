...
### device_state · 查全部设备当前开着还是关着
何时：问**全部设备**的状态，没有点名是灯：『家里还有什么没关』『哪些设备还开着』『都关干净了吗』。
辨析：★★用户句子里出现「灯」字时**不要用这个**，用 lights_state。这个是不限类型的全设备清单。
     ★这是查状态，不是控制。问『开着吗』别去开它。

### lights_state · 查哪些灯还开着
何时：问题里出现了「灯」这个字，问的是灯的开关状态。『家里哪些灯还开着』『灯都关了吗』『还有灯亮着没』『有没有灯忘关了』。
辨析：★★判据很简单：**用户说了"灯"就用这个**。只报灯（含浴霸），绝不把新风、摄像头、零冷水列出来。
     ★和 all_lights_off 的区别：那个是"把灯关掉"（执行），这个是"哪些灯还开着"（询问）。
     ★和 device_state 的区别：那个报全部设备，只在用户问"哪些设备/什么还开着"、没点名灯时才用。
...
### all_lights_off · 关掉所有灯
何时：『把所有灯关了』『全屋灯都关掉』『灯全关了吧』——一次关掉全部照明。
辨析：★和 scene_sleep 不同：睡觉场景还会拉窗帘、关空调；这个只管灯。

### all_ac_on · 打开所有空调
何时：『所有空调都开开』『把家里空调全打开』。
辨析：★用户说"所有/全部"才用这个；只说『打开空调』是就近那一台。

### all_ac_off · 关掉所有空调
何时：『空调全关了』『所有空调关掉』『家里空调都关上』。
辨析：★同上，要有"所有/全部/都"这类词。

### all_fresh_air_off · 关掉新风
何时：『新风都关了』『把新风关掉』。
辨析：★全屋新风总开关。
...
### music_pause · 暂停/继续播放
何时：『暂停』『别放了』『停一下』『继续播』。
辨析：★music_play 是开始放歌，这个是暂停正在播的。

### music_next · 下一首
何时：『下一首』『换一首』『切歌』。
辨析：★不是重新开始播放。

### ac_temp_unsupported · 空调调温（暂不支持，如实说明）
何时：要求调空调温度、设定具体度数。『空调调高点』『调到26度』『空调温度低一点』。
辨析：★★家里空调只接了开关（红外/插座控制），climate 实体全部离线，**调不了温度**。
     必须如实告诉用户做不到，绝对不能拿"关空调"(ac_off)或"开空调"(ac_on)顶替——
     用户说"调高点"结果被关掉，是实际发生过的事故。
